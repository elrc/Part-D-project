...
trsa,trta,trpa,tbsa,tbta,tbpa,ct,at,dv,ts = [],[],[],[],[],[],[],[0],[],[]
sv,av,tv,atv,pv,apv = [],[],[],[],[],[]
...
while(True):
    ...
    if rsv != 0 or rtv != 0 or rpv != 0 or bbsv != 0 or bbtv != 0 or bbpv != 0:
        vmax = max(vsmax,vtmax,vpmax,bvsmax,bvtmax,bvpmax)
        vavg = (vsavg + vtavg + vpavg + bvsavg + bvtavg + bvpavg) / (rsv + rtv + rpv + bbsv + bbtv + bbpv)
    if rsfrc != 0 or rtfrc != 0 or rpfrc != 0 or bsfrc != 0 or btfrc != 0 or bpfrc != 0:
        frmax = max(rsfrmax,rtfrmax,rpfrmax,bsfrmax,btfrmax,bpfrmax)
        fravg = (rsfravg + rtfravg + rpfravg + bsfravg + btfravg + bpfravg) / (rsfrc + rtfrc + rpfrc + bsfrc + btfrc + bpfrc)
...
print("[INFO] saving variables...")

var_vel = np.array([smax,savg,vmax,vavg,frmax,fravg])

data = zip(ts,at,ct,dv,rsvat,svt,avt,rsft,rtvat,tvt,atvt,rtft,rpvat,pvt,apvt,rpft,bsvat,bsvt,bavt,bsft,btvat,btvt,batvt,btft,bpvat,bpvt,bapvt,bpft)
